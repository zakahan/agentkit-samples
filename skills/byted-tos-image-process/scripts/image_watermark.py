...
Watermark parameters vary by watermark type (text/image/mix) and are subject to
TOS official documentation. This script intentionally avoids hardcoding all
parameter names.

Pass watermark parameters using repeated `--kv key=value`, and the script will
append them as `key_value` segments in the process string.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply watermark via TOS process=image/watermark")
    ...
    parser.add_argument("--kv", action="append", default=[], help="Watermark option: key=value")
    ...
    if not args.kv:
        print("[ERROR] At least one --kv key=value is required for watermark parameters.", file=sys.stderr)
        sys.exit(1)
