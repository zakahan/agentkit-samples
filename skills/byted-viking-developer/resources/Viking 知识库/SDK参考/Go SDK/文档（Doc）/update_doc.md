...
本示例演示了知识库 Go SDK 中 UpdateDoc 的基础使用方法，通过指定知识库名称、文档 ID 和新文档名称实现文档名称修改，使用前需配置 API Key 鉴权参数。
```Go
package main
...
func main() {
    ...
    docID := "your_doc_id"
    docName := "new_doc_name"
    rsp, err := collection.UpdateDoc(ctx, docID, docName)
    if err != nil {
        fmt.Printf("UpdateDoc failed, err: %v\n", err)
        return
    }
    fmt.Printf("UpdateDoc success, Response: %v\n", rsp)
}
```
