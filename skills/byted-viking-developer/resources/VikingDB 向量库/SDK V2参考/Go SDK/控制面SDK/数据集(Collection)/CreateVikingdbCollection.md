...
| CollectionName | string | 是 | 数据集的名称，对应 API 字段 `CollectionName` <br>  <br> * 只能使用英文字母、数字、下划线_，并以英文字母开头，不能为空 <br> * 长度（字节）要求：[1, 128] <br> * 同账号下，所有 Collection 名称不能重复 <br> * 同账号下，Collection 数量不超过 200 个 |
| Description | string | 否 | 数据集的描述 <br> 最长 65535 字节 |
| Fields | []FieldForCreateVikingdbCollectionInput | 是 | 数据集中的字段。 <br> 限制：数据集内最多 200 个字段。 |
...
| DefaultValue | interface{} | 否 | 字段内容默认值。 <br> 注意：vector/sparse_vector/text/image/video 类型字段不支持默认值。 |
