...
| CollectionName | string | 2选1 | 待删除集合名称，对应 API 字段 `CollectionName`。 <br>  <br> * 以字母开头，仅可包含字母、数字、下划线，长度 1-128。 <br> * 服务端允许与 `ResourceId` 二选一。 |
| ResourceId | string |  | 集合资源 ID，对应 API 字段 `ResourceId`。可与 `CollectionName` 组合使用，帮助在控制面追踪。 |
# 返回参数
| 参数 | 类型 | 示例值 | 描述 |
| --- | --- | --- | --- |
