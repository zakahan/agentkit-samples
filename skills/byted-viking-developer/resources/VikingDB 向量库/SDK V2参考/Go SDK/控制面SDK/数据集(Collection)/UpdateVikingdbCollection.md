...
# 方法定义
Go SDK 通过 `vikingdb.New(sess)` 创建的客户端实例调用 `UpdateVikingdbCollection(input)` 方法发起集合更新请求，input 参数类型为 `vikingdb.UpdateVikingdbCollectionInput`，包含集合名称、描述、项目名称和新增字段定义等配置。
# **请求参数**
请求参数是 UpdateVikingdbCollectionInput，UpdateVikingdbCollectionInput 包含的参数如下表所示。
| 参数 | 类型 | 是否必选 | 描述 |
| --- | --- | --- | --- |
| ProjectName | string | 否 | 项目名称，对应 API 字段 `ProjectName`，默认 default。 |
| CollectionName | string | 二选一 | 集合名称，对应 API 字段 `CollectionName`。以字母开头，仅可包含字母、数字、下划线，长度 1-128。可与 `ResourceId` 二选一。 |
| ResourceId | string | 二选一 | 集合资源 ID，对应 API 字段 `ResourceId`。与 `CollectionName` 二选一，推荐同时填写确保准确。 |
...
| 参数 | 类型 | 是否必选 | 描述 |
| --- | --- | --- | --- |
| FieldName | string | 是 | 字段名。 <br> 限制：英文字母、数字、或下划线。且必须以英文字母开头。长度不超过 64 Byte。 |
