# 概述
`ListVikingdbIndex` 用于查询与数据集（Collection）关联的索引（Index）列表。
# 方法定义
Go SDK 通过 `vikingdb.New(sess)` 创建的客户端实例调用 `ListVikingdbIndex(input)` 方法发起索引列表查询请求，input 参数类型为 `vikingdb.ListVikingdbIndexInput`，包含查询所需的参数。
# **请求参数**
| 参数 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
...
| Status | []string | 状态 |
# 返回参数
Go 调用执行上述操作，返回 Index 实例列表。Index 实例包含的属性如下表所示。
...
|  |  | DiskannM | int | diskann参数，表示邻居节点个数。 <br>  <br> * 当 index_type 配置为 diskann时可选配置。 |
...
|  | ShardPolicy |  | string | 索引分片类型，auto 为自动分片、custom 为自定义分片。 |
