...
| IDs | []interface{} | 2选1 | 删除数据的主键列表（主键为 int64 或 string）。最多 100 条。 <br> 注意： <br>  <br> * 若为请求参数非法（4xx 类型），则会全部失败。 |
|  |  |  |  |
| DelAll | bool |  | 为 true 时，删除所有数据；默认为 false。 <br> 此接口删除所有数据，并不能立刻同步到索引，因此，在一段时间内（约 5 分钟），索引内仍可检索到数据。 |
# 返回参数
