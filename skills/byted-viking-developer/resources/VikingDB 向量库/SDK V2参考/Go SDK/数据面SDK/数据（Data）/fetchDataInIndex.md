# 概述
FetchDataInIndex 用于 Index 数据查询。根据主键 ID，在指定的 Index 中查询单条或多条数据，单次最多可查询 100 条数据。
Collection 数据写入/删除后，Index 数据更新存在同步延迟，一般在 10 秒左右，无法立即在 Index 中查询到。

# **请求参数**
| 参数名 | 类型 | 必选 | 备注 |
| --- | --- | --- | --- |
| CollectionName | string | 2选1 | Collection 名称，与 ResourceID 二选一。 |
| ResourceID | string |  | Collection 资源 ID。 |
| IndexName | string | 是 | 索引名称。 |
| IDs | []interface{} | 是 | 点查的主键列表，最多 100 条。 |
