# 概述
searchByVector 用于向量检索：根据查询向量，搜索与其距离最近的 limit 个向量。
...
# **请求参数**
请求参数是 SearchByVectorRequest，SearchByVectorRequest 实例包含的参数如下表所示。
| **参数** | **类型** | **是否必选** | **参数说明** |
| --- | --- | --- | --- |
| denseVector | List[Float] | 是 | 用于检索的稠密向量。 |
| sparseVector | Map[String, Float] | 否 | 检索的稀疏向量。 |
| filter | Map[String, Object] | 否 | 过滤条件，详见**标量过滤** <br>  <br> * 默认为空，不做过滤。 <br> * 过滤条件包含 must、must_not、range、range_out 四类查询算子，包含 and / or 两种对查询算子的组合。 |
| outputFields | List[String] | 否 | 要返回的标量字段列表。 <br>  <br> 1. 用户不传 outputFields 时，返回所有标量字段。 <br> 2. 用户传一个空列表时，不返回标量字段。 <br> 3. outputFields 格式错误，或者字段不是 collection 里的字段时，接口返回错误 |
