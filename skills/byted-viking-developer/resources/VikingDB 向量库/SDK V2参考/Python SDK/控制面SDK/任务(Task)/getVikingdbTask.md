# 概述
查询指定 Task 的详情信息与执行进度。
# 方法定义
Python SDK 通过 `VIKINGDBApi().get_vikingdb_task(request)` 调用，其中 `request` 类型为 `volcenginesdkvikingdb.GetVikingdbTaskRequest`。
# 请求参数
| **参数** | **类型** | **是否必选** | **说明** |
| --- | --- | --- | --- |
...
| task_status | str | 任务状态，对应 API 字段 `TaskStatus`，取值可为 `init`/`queued`/`confirm`/`confirmed`/`running`/`done`/`fail`。 |
...
| **字段** | **类型** | **描述** |
| --- | --- | --- |
| collection_name | str | 任务作用的 Collection 名称，API 字段 `CollectionName`。 |
