# 概述
update_vikingdb_collection 用于为指定数据集 Collection 追加字段。
Collection 支持新增字段（fields）；已定义字段（fields）不支持修改，仅支持修改数据集描述（description）。
...
# **请求参数**
请求参数为 UpdateVikingdbCollectionRequest，包含的参数如下表所示。
| 参数 | 类型 | 是否必选 | 描述 |
| --- | --- | --- | --- |
| project_name | str | 否 | 项目名称，对应 API 字段 `ProjectName`，默认 default。 |
| collection_name | str | 至少填写一个 | 集合名称，对应 API 字段 `CollectionName`。以字母开头，仅可包含字母、数字、下划线，长度 1-128。与 `resource_id` 至少填写一个，可同时填写。 |
| resource_id | str |  | 集合资源 ID，对应 API 字段 `ResourceId`。与 `collection_name` 至少填写一个，推荐同时填写确保准确。 |
...
| field_type | str | 是 | 字段类型，对应 API 字段 `FieldType`，支持 vector/sparse_vector/string/int64/float32/bool/list/list/text/image/video/date_time/geo_point。 |
