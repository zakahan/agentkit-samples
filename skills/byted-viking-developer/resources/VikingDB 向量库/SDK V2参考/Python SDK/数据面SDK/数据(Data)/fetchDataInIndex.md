# 概述
fetchDataInIndex 用于 Index 数据查询。根据主键 id，在指定的 Index 查询单条或多条数据，单次最多可查询 100 条数据。
Collection 数据写入/删除后，Index 数据更新时间有同步延迟，一般在 10s 左右，不能立即在 Index 查询到。
...
| ids | List[Any] | 是 | 点查的主键列表，最多 100 条。 |
...
request = FetchDataInIndexRequest(
    ids=[4798786918981052481, 8517775955036588997],  # 可从控制台或 searchByRandom API 获取 ID
